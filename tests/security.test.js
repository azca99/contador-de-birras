...
describe("PUBLICUSERS", () => {
  ...
  it("lectura p�blica autenticada si procede", async () => {
    await testEnv.withSecurityRulesDisabled(async (context) => {
...
describe("GROUPS", () => {
  ...
  it("admin crea grupo v�lido", async () => {
    const db = testEnv.authenticatedContext("alice").firestore();
  ...
  it("miembro a�ade tercero falla", async () => {
    await testEnv.withSecurityRulesDisabled(async (context) => {
      await context.firestore().collection("groups").doc("g1").set({
        adminUid: "alice", members: ["alice", "bob"], name: "Group 1", createdAt: 12345
      });
  ...
  it("creaci�n con adminUid ajeno falla", async () => {
    const db = testEnv.authenticatedContext("bob").firestore();
